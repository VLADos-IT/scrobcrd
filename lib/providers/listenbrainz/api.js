...
async function getRecentListens(user, limit) {
    ...
    const isPlayingNow = playingRes.status === 200 && playingRes.data?.payload?.playing_now;
    const playingTrack = isPlayingNow ? playingRes.data?.payload?.listens?.[0] : null;

    const listens = listensRes.status === 200 ? listensRes.data?.payload?.listens : null;
    if (!Array.isArray(listens) || listens.length === 0) {
        if (!playingTrack) return null;
        listens.push(playingTrack);
    } else if (playingTrack) {
        const topMeta = listens[0].track_metadata;
        const playMeta = playingTrack.track_metadata;
        const isAlreadyTop = topMeta.track_name === playMeta.track_name && topMeta.artist_name === playMeta.artist_name;
        
        if (!isAlreadyTop) {
            listens.unshift(playingTrack);
            listens.pop();
        }
    }
