...
const { fetchRecentTracksList } = require('./fetchers/recent-list');
...
async function fetchLastFmData(user, mode = 'smart', range = 'all', limit = 5) {
	...
	const cacheKey = `${user}:${mode}:${range}:${mode === 'list' || mode === 'history' ? limit : ''}`;
	...
	if (mode === 'history') {
		const result = await fetchRecentTracksList(user, limit);
