...
function isNowPlayingRow(rowHtml) {
    return /now[\s-]*playing/i.test(rowHtml)
        || /now[\s-]*scrobbling/i.test(rowHtml)
        || /class="[^"]*\b(chartlist-row--?now-playing|chartlist-now-playing|now-playing)\b[^"]*"/i.test(rowHtml);
}

function parseChartlistRows(html, defaultUrl, type, limit, options = {}) {
    const { detectNowPlaying = false } = options;
    const rows = [];
    let searchFrom = 0;
    ...
    while (rows.length < limit) {
        ...
        if (detectNowPlaying && rows.length === 0) {
            parsed.isPlaying = isNowPlayingRow(rowHtml);
        }
...
module.exports = { parseChartlistRow, parseChartlistRows, isNowPlayingRow };
