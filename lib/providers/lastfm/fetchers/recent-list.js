...
const { backfillMissingImages } = require('./image-fallback');
...
async function fetchRecentTracksList(user, limit = 5) {
	...
	if (API_KEY) {
		try {
			...
			let mappedTracks = tracks.slice(0, safeLimit).map((track, index) => ({
				rank: index + 1,
				track: track.name,
			...
			mappedTracks = await backfillMissingImages(user, mappedTracks, safeLimit);
