...
/**
 * Renders the top-N tracks list card.
 * @param {object} params
 * @param {Array<object>} params.tracks
 * @param {string} params.width
 * @param {string} params.bgFill
 * @param {string} params.textColor
 * @param {string} params.artistColor
...
function renderList(params) {
    ...
    const availableTextWidth = numericWidth - PADDING * 2 - THUMB_SIZE - 14 - 46; // reserve space for rank + playcount
    ...
    const rows = tracks.map((t, i) => {
        ...
        <a xlink:href="${safeUrl}" target="_blank">
            ...
            ${playcountLabel ? `<text x="${numericWidth - PADDING}" y="${y + ROW_HEIGHT / 2 + 4}" text-anchor="end" class="playcount-row" fill="${preset.accent}">${playcountLabel}</text>` : ''}
    ...
    const outerBorder = preset.pixelBevel ? '' : `<rect width="${numericWidth}" height="${height}" rx="${cornerRadius}" fill="none" stroke="${preset.border || dividerColor}" stroke-width="1.5" />`;
    ...
    <svg width="${numericWidth}" height="${height}" viewBox="0 0 ${numericWidth} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        ...
        <g clip-path="url(#list-card-clip)">
            ...
            ${pixelOverlay}
            ${chrome}
            <text x="${PADDING}" y="${headerY}" class="list-header" fill="${preset.accent}">${headerLabel}</text>
            ${rows}
            ${outerBorder}
        </g>
    </svg>`;
