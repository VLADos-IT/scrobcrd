...
function defaultCard({ width, height, bgFill, borderRadius, padding, imageSize, url, profileUrl, imageBase64, headerText, headerColor, textColor, artistColor, safeTrack, safeArtist }) {
    ...
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        ...
        <a xlink:href="${url}" target="_blank">
            ${imageBase64 
                ? `<image x="${padding}" y="${padding}" width="${imageSize}" height="${imageSize}" xlink:href="${imageBase64}" rx="4" />` 
                : `<rect x="${padding}" y="${padding}" width="${imageSize}" height="${imageSize}" rx="4" fill="${headerColor}" opacity="0.1" />`}
    ...
    <svg width="${params.width}" height="${params.height}" viewBox="0 0 ${params.width} ${params.height}" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        ...
        <g clip-path="url(#narrow-card-clip)">
            ...
            <a xlink:href="${params.url}" target="_blank">
                ${params.imageBase64 
                    ? `<image x="${padding}" y="${coverY}" width="${coverSize}" height="${coverSize}" xlink:href="${params.imageBase64}" rx="4" />` 
                    : `<rect x="${padding}" y="${coverY}" width="${coverSize}" height="${coverSize}" rx="4" fill="${params.headerColor}" opacity="0.1" />`}
...
function renderTiny({ width, height, bgFill, borderRadius, url, profileUrl, imageBase64, headerColor }) {
    ...
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
        ...
        <a xlink:href="${url}" target="_blank">
            ${imageBase64 
                ? `<image x="${coverX}" y="${coverY}" width="${coverSize}" height="${coverSize}" xlink:href="${imageBase64}" rx="10" />` 
                : `<rect x="${coverX}" y="${coverY}" width="${coverSize}" height="${coverSize}" rx="10" fill="${headerColor || '#d1170e'}" opacity="0.1" />`}
        </a>
    </svg>`;
}
